...
const UserAchievements = () => {
    ...
    const { data: rewardsData, isLoading, error } = useQuery({
      ...
      const completedMissions = missionTypes.reduce((total, type) => {
        ...
        return total + (completed === parseInt(rewardsData[type].split('/')[1]) ? 1 : 0);
      }, 0);
  ...
  return (
    <div className="p-4 border-b max-w-sm mx-auto">
      <div className="flex items-center mb-4">
        <Trophy strokeWidth={1.5} className="w-auto h-6 text-purpleWaki mr-2" />
        <p className="text-sm font-medium">{t("profile.achievements")}</p>
      </div>
      
      <div className="mb-4">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-gray-700">{t("profile.quests")}</span>
          <span className="text-sm font-medium text-gray-700">{completedMissions}/{totalMissions}</span>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center">
          ...
          <span className="text-sm">{t("profile.pointsEarned")}: {rewardsData.total_points_awarded}</span>
        </div>
        <div className="flex items-center">
          <Medal className="w-5 h-5 text-green-500 mr-2" />
          <span className="text-sm">{t("profile.totalPoints")}: {rewardsData.total_points}</span>
        </div>
      </div>
    </div>
  )
